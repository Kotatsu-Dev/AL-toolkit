async function addFollowCount(){
	...
	const userData = await anilistAPI("query($name:String){User(name:$name){id}}", {
		...
		cacheKey: "altoolkitIDlookup" + decodeURIComponent(URLstuff[1]).toLowerCase(),
		duration: 5*60*1000
	});
	if(userData.errors){
		return
	...
	const insertCount = function(data, id, pos){
		...
		if(target){
			...
			create("span",[id,"altoolkitCount"],followCount,target.children[pos]);
		}
	}
	insertCount(followerData, "#altoolkitFollowersCount", 2)
	insertCount(followingData, "#altoolkitFollowingCount", 1)
	return
}
